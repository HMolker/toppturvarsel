...
// The conditions page: the real map.
{
  ...
  console.log('desc chips:', await page.locator('#descList .dchip').count());
  await page.click('#descList [data-rmdesc="1"]');
  console.log('after removing one:', await page.evaluate(() => JSON.stringify(window.fjallskredTerrain.state.tour.names)), '· run buttons:', JSON.stringify(await page.$$eval('#runsOut .runitem button', (els) => els.map((e) => e.textContent))));
  await page.click('#runsOut [data-addrun="1"]');
