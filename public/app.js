...
const DIFF_NAMES = { novice: 'novice', easy: 'easy', intermediate: 'intermediate', advanced: 'advanced', expert: 'expert', freeride: 'freeride / off-piste', unknown: 'not graded' };
...
function factsHtml(x) {
  ...
  if (x.runCount) {
    ...
    cards.push(`<li class="wide"><b>${x.runCount} runs · ${x.runKm} km</b><span>of mapped piste${extras ? ` · ${extras}` : ''}</span><div class="rschips">${chips}</div></li>`);
  }
  if (x.liftCount) {
  ...
  return `<ul class="rsfacts">${cards.join('')}</ul>${liftTable}${runList}${eatList}` +
    `<p class="note">From OpenStreetMap (© OpenStreetMap contributors): what is mapped, which can differ from the resort's own figures. Lift heights from the terrain model at the stations. Runs mapped in pieces count once by name and number.</p>`;
}
